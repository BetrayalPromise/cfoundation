...
# 27 "testcstring.c"
 if (1) {
  char * p0 = cstringinit("", 1);
  char * p1 = cstringinit("", 0);
  char * p2 = cstringinit("\00000", 1);
  char * p3 = cstringinit("\00000", 0);
 ...
 if (1) {
  char * p = ((void*)0); do { char temp[2 * sizeof(long) + 1]; *(long *)(temp) = 1; *(long *)(temp + sizeof(long)) = 1; temp[2 * sizeof(long)] = 'y'; p = temp + 2 * sizeof(long); } while(0);;
...
void testcstringtelescope() {
    char * p = cstringinit(0x00, (0));
 cstringtelescope(&p, 1, 2);
 cstringdescribe(p, 0b100);
}
...
# 162 "testcstring.c"
 if (1) {
  char * p0 = cstringinit("0000000", (0));
  char * p1 = cstringinit("00", (0));
  for (int i = -1; i < 10; i ++) {
   long index = cstringindex(p0, i, p1, 1);
   printf("i = %d, index = %ld\n", i, index);
  }

  for (int i = -1; i < 10; i ++) {
   long index = cstringindex(p0, i, p1, 0);
   printf("i = %d, index = %ld\n", i, index);
  }
 }
...
# 205 "testcstring.c"
 if (1) {
  char * p = cstringinit("abcdefgh", (0));
  cstringcleans(p, position, ((void*)0), (8 == 1 ? ("-1")[0] != 0 : 8), -1, -1, 5, 5, 3, 3, 0, 0);
...
void testcstringunique() {
 char * p0 = cstringinit("0523541007840000", (0));
 cstringunique(p0, 0);
