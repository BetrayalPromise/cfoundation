...
# 84 "/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk/usr/include/i386/types.h" 3 4
...
# 96 "./cstring.h"
extern char * cstringinit(char * str, _Bool ctl);
# 107 "./cstring.h"
...
extern void cstringtelescope(char ** pcstr, _Bool ctl, size_t m);
# 131 "./cstring.h"
...
extern _Bool cstringcompare(char * cstr, char * data);
...
extern _Bool cstringinsert(char * cstr, long index, ...);
...
extern _Bool cstringcatenate(char * cstr, ...);
...
extern _Bool cstringcleans(char * cstr, search_t t, int (* cfg)(long idx), size_t ps, ...);
# 190 "./cstring.h"
...
extern _Bool cstringunique(char * cstr, _Bool flag);
...
extern _Bool cstringchange(char * cstr, long pos, ...);
...
static _Bool cstringexcise(char * cstr, char * data);
static _Bool cstringdelete(char * cstr, char c);
static _Bool cstringremove(char * cstr, long index);

static _Bool cstringcheck(char * cstr) {
 return cstr == ((void*)0) ? 0 : 1;
}
...
char * cstringinit(char * str, _Bool ctl) {
    long vstep = basesize();
 long lstep = basesize();
 long volume = 32;
 long length = 0;
 ...
 if (str == ((void*)0)) {
  ...
  length = ctl == 1 ? strlen(str) + 1 : strlen(str);
 }
 while (volume <= length) {
  volume *= 2;
 }
...
char * cstringcopy(char * cstr) {
 if (!cstringcheck(cstr)) { return ((void*)0); }
 char * index = ((void*)0);
 __builtin___memcpy_chk (index, cstr, cstringlength(cstr) + basesize(), __builtin_object_size (index, 0));
...
_Bool cstringcompare(char * cstr, char * data) {
 if (!cstringcheck(cstr)) { return 0; }
...
void cstringtelescope(char ** pcstr, _Bool ctl, size_t m) {
 ...
 __builtin___memcpy_chk (space, *pcstr, 2 * basesize() + volume, __builtin_object_size (space, 0));
 printf("cstring: (%p) -> (%p)\n", * pcstr, space);
...
_Bool cstringinsert(char * cstr, long index, ...) {
 uint64_t rdi = 0x0;
 ...
 va_list list;
 __builtin_va_start(list, index);

 uint64_t result = __builtin_va_arg(list, uint64_t);
 ...
 if (0x00 <= result && result <= 0xff) {
  ...
  while (length + 1 > volume) {
   cstringtelescope(&cstr, 1, 2);
   volume = cstringvolume(cstr);
  }

  __builtin___memmove_chk (cstr + index, cstr, length - index, __builtin_object_size (cstr + index, 0));
  __builtin___memmove_chk (cstr + index, &data, 1, __builtin_object_size (cstr + index, 0));
  ...
  __builtin___memmove_chk (cstr + cstringlength(data), cstr, cstringlength(cstr) - index, __builtin_object_size (cstr + cstringlength(data), 0));
  __builtin___memmove_chk (cstr + index, data, cstringlength(data), __builtin_object_size (cstr + index, 0));
...
_Bool cstringcatenate(char * cstr, ...) {
 if (!cstringcheck(cstr)) { return 0; }

 va_list list;
 __builtin_va_start(list, cstr);
 ...
 if (0x00 <= result && result <= 0xff) {
  ...
  __builtin___memmove_chk (cstr + length, &data, 1, __builtin_object_size (cstr + length, 0));
  setcstringlength(cstr, length + 1);
  ...
  __builtin___memmove_chk (cstr + length, data, cstringlength(data), __builtin_object_size (cstr + length, 0));
  setcstringlength(cstr, length + cstringlength(data));
  __builtin_va_end(list);
  return 1;
 }
...
_Bool cstringcleans(char * cstr, search_t t, int (* cfg)(long idx), size_t ps, ...) {
 if (!cstringcheck(cstr)) { return 0; }
 va_list list;
 __builtin_va_start(list, ps);
 if (cstringlength(cstr) < 1 ) { return 0; }
 ...
 case position: {
  long source[ps];
  __builtin___memset_chk (source, -1, sizeof(long) * ps, __builtin_object_size (source, 0));
  long count = 0;
  long length = cstringlength(cstr);
  for (int i = 0; i < ps; i ++) {
   long result = cfg && cfg(i) == 8 ? __builtin_va_arg(list, long) : __builtin_va_arg(list, int);
 ...
 case cstring: {
  for (long i = 0; i < ps; i ++) {
   long value = __builtin_va_arg(list, long);
...
static _Bool cstringexcise(char * cstr, char * data) {
 ...
 __builtin___memset_chk (indexes, -1, length / size * sizeof(int), __builtin_object_size (indexes, 0));
 int index = 0, count = 0;
 int i = 0, j = 0;
 ...
 for (int i = count - 1; i >= 0; i --) {
  ...
  __builtin___memmove_chk (cstr + indexes[i] - size + 1, cstr + indexes[i] + 1, length - indexes[i], __builtin_object_size (cstr + indexes[i] - size + 1, 0));
 }
 setcstringlength(cstr, length - count * size);
 return 1;
}

_Bool cstringexcises(char * cstr, long ps, char * data, ...) {
 if (!cstringcheck(cstr)) { return 0; }
 long length = cstringlength(cstr);
 va_list list;
 __builtin_va_start(list, data);
 long source[ps];
 __builtin___memset_chk (source, -1, sizeof(char *) * ps, __builtin_object_size (source, 0));
 source[0] = (long)data;
 long scount = 1;
 for (int i = 1; i < ps; i ++) {
  long result = __builtin_va_arg(list, long);
...
static _Bool cstringdelete(char * cstr, char c) {
 ...
 for (int i = 0; i < count; i ++) {
  __builtin___memmove_chk (cstr + indexes[i] - i, cstr + indexes[i] + 1 - i, length - indexes[i] - 1 + i, __builtin_object_size (cstr + indexes[i] - i, 0));
  __builtin___memmove_chk (cstr + length - 1, &c, 1, __builtin_object_size (cstr + length - 1, 0));
 }
 setcstringlength(cstr, length - count);
 return 1;
}

_Bool cstringdeletes(char * cstr, size_t ps, int c, ...) {
 if (!cstringcheck(cstr)) { return 0; }
 long length = cstringlength(cstr);
 va_list list;
 __builtin_va_start(list, c);
 int source[ps];
 __builtin___memset_chk (source, -1, sizeof(int) * ps, __builtin_object_size (source, 0));
 source[0] = c;
 long scount = 1;
 for (int i = 1; i < ps; i ++) {
  long result = __builtin_va_arg(list, int);
  if (result < length - 1 && result >= 0) {
   _Bool flag = 0;
   for (int j = 0; j < i; j ++) {
    if (source[j] != result) { continue;}
...
static _Bool cstringremove(char * cstr, long index) {
 ...
 if (index > length - 1 || index < 0) {
  ...
  __builtin___memmove_chk (cstr + index, cstr + index + 1, length - index - 1, __builtin_object_size (cstr + index, 0));
  setcstringlength(cstr, length - 1);
  return 1;
 }
}

_Bool cstringremoves(char * cstr, size_t ps, int index, ...) {
 if (!cstringcheck(cstr)) { return 0; }
 long length = cstringlength(cstr);
 va_list list;
 __builtin_va_start(list, index);
...
long cstringindex(char * cstr, long times, ...) {
 ...
 if (0x00 <= result && result <= 0xff) {
  ...
  uint64_t flag = __builtin_va_arg(list, uint64_t) != 0 ? 1 : 0;
  _Bool ctl = 0;

  if (flag) {
   for (int i = 0; i < size - 1; i ++) {
    data[0] == data[i] ? ({ ctl = 1; continue; }) : ({ ctl = 0; break; });
   }
  }
...
long cstringindexcount(char * cstr, ...) {
 ...
 if (0x00 <= result && result <= 0xff) {
  ...
  long index = 0;
  long size = cstringlength(data);
  int64_t flag = __builtin_va_arg(list, uint64_t) != 0 ? 1 : 0;
  _Bool ctl = 0;
...
_Bool cstringunique(char * cstr, _Bool flag) {
 if (!cstringcheck(cstr)) { return 0; }
 long length = cstringlength(cstr);
 ...
 __builtin___memset_chk (buffer, 0, sizeof(buffer), __builtin_object_size (buffer, 0));
 __builtin___memset_chk (delete, 0, sizeof(buffer), __builtin_object_size (delete, 0));
   ...
   flag == 1 ? ({}) : ({
    if(delete[cstr[i]] == 0) {
     ...
     for (int k = 0; k < j; k ++) {
      if (cstr[k] == cstr[i]) {
       char c = cstr[i];
       __builtin___memmove_chk (cstr + k, cstr + k + 1, j - k - 1, __builtin_object_size (cstr + k, 0));
       cstr[i - 1] = c;
       --j; break;
      }
     }
...
_Bool cstringchange(char * cstr, long pos, ...) {
 if (!cstringcheck(cstr)) { return 0; }

 va_list list;
 __builtin_va_start(list, pos);
 uint64_t result = __builtin_va_arg(list, uint64_t);
 long length = cstringlength(cstr);
 char * data = ((void*)0);
 if (0x00 <= result && result <= 0xff) {
  data = ((void*)0); do { char temp[2 * sizeof(long) + 1]; *(long *)(temp) = 1; *(long *)(temp + sizeof(long)) = 1; temp[2 * sizeof(long)] = result; data = temp + 2 * sizeof(long); } while(0);;
 } else {
  data = ((void*)0); do { char temp[2 * sizeof(long) + cstringlength((char *)result)]; for(int i = 0; i < cstringlength((char *)result); i ++) { temp[2 * sizeof(long) + i] = ((char *)(char *)result)[i]; } data = temp + 2 * sizeof(long); } while(0);;
 }
